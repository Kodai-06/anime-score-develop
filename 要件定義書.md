...
レビューが投稿された場合、外部API（Annict）からレビューされたアニメのデータを自動的に取得・保存し、データベースを自己拡張していく機能を特徴とする。
初めての開発なので余計な機能は実装しない
...
| **データベース** | PostgreSQL(pgx,sqlxを使う) | データ永続化 |
...
クエリパフォーマンス向上のため、以下のインデックスを設定：

| テーブル | カラム | 用途 |
| :--- | :--- | :--- |
| `reviews` | `user_id` | ユーザー別レビュー検索高速化 |
| `reviews` | `anime_id` | アニメ別レビュー検索高速化 |
| `animes` | `title` | 作品検索高速化 |
| `animes` | `annictId` | Annict ID による重複チェック高速化 |
...
├── cmd/
│   └── api/
│       └── main.go           # 【起動担当】サーバーの起動、DB接続設定
├── internal/                 # 【アプリの主要コード】外部からimportされない安全地帯
│   ├── models/               # 【型定義】DBやAPIのデータ構造 (struct)
│   ├── handlers/             # 【受付担当】HTTPリクエストを受け取り、レスポンスを返す (Gin)
│   ├── services/             # 【司令塔】ビジネスロジック (「DBになければAPIを叩く」等の判断)
│   └── repositories/         # 【倉庫担当】DB操作や外部APIとの実際の通信
├── pkg/                      # 【便利ツール】汎用的なライブラリ (日付変換など)
├── db/                       # 【DB管理】マイグレーションファイルなど
├── go.mod
└── .env